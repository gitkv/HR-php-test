...
tinker: up ## Start shell into application container
	$(docker_compose_bin) exec -T "$(APP_CONTAINER_NAME)" php artisan tinker

install: up ## Install application dependencies into application container
	$(docker_compose_bin) exec -T "$(APP_CONTAINER_NAME)" composer install --no-interaction --ansi --no-suggest
	$(docker_compose_bin) exec -T "$(APP_CONTAINER_NAME)" cp ./.env.example.for-docker ./.env
	$(docker_compose_bin) exec -T "$(APP_CONTAINER_NAME)" artisan key:generate

watch: up ## Start watching assets for changes (node)
	$(docker_compose_bin) run --rm "$(NODE_CONTAINER_NAME)" npm run watch

init: install ## Make full application initialization (install, migrate, seed, build assets, etc)
	$(docker_compose_bin) exec -T "$(APP_CONTAINER_NAME)" artisan migrate --force --no-interaction -vvv
	$(docker_compose_bin) exec -T "$(APP_CONTAINER_NAME)" artisan db:seed --force -vvv

test: up ## Execute application tests
	$(docker_compose_bin) exec -T "$(APP_CONTAINER_NAME)" vendor/bin/phpunit