...
Для Laravel, используя Docker:
- подготовить локально БД `MySql`, имя пользователя, пароль и имя базы указаны в файле `docker-compose.yml`, при необходимости поменять
- если необходимо резолвить домен на локальной машине, отличный от `localhost`, необходимо добавить строку в файл `hosts` - `sudo sh -c "echo '127.0.0.1 hr-php-test.loc' >> /etc/hosts"` 
- `make init` (выполняется только 1 раз, для запуска развернутого проекта `make up`, PS: для удобства над докером настроена обертка в виде `makefile`)
- для выполнения artisan комманд приложения `make shell` (переход внутрь контейнера)
- получить справку по командам - `make`
- для остановки приложения `make down`
